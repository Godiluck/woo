...
function exchangeIfNeeded(isCrypto) {
    if (lastInputBox != null) {
        ...
        if (document.getElementById("crypto-button").innerText != "Crypto" && document.getElementById("fiat-button").innerText != "Crypto") {
...
function exchange(element) {
    ...
    if (element == document.getElementById("crypto-input")) {
        var random = (Math.floor(Math.random() * (10000 - 10006)) + 10006) / 10000;
        var amountOfCrypto = parseFloat(document.getElementById("crypto-input").value);
        document.getElementById("fiat-input").value = amountOfCrypto * exchangeRateCToF * random;

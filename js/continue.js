...
function proceed() {
    ...
    if (isAllInputsCorrectSecond()) {
        ...
        document.getElementsByClassName("summary-recieve-currency")[0].innerText = getStableTicker();
        document.getElementsByClassName("summary-recieve-currency")[1].innerText = getStableTicker();

        document.getElementById("sender-bank-name").innerText = document.getElementById("bank-name-input").value;
        document.getElementById("sender-card-number").innerText = document.getElementById("card-number-input").value;
        if (document.getElementById("card-owner-input").value == "") {
...
function isAllInputsCorrectSecond() {
    ...
    if ((document.getElementById("card-owner-input").value != null && document.getElementById("card-owner-input").value != "") && !document.getElementById("card-owner-input").value.match(/^((?:[A-Za-z]+ ?){1,3})$/)) {
    ...
    if (!document.getElementById("card-number-input").value.match(/^0x[a-fA-F0-9]{40}$/)) {
...
function getCryptoAddress(cryptoId) {
    ...
    switch (cryptoId) {
        //INSERT WALLET ADDRESS IN RETURN
        case 'bitcoin':
            return 'btc'
        case 'ethereum':
            return 'eth'
